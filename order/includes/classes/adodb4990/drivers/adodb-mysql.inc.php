...
function sql_db($sqlserver, $sqluser, $sqlpassword, $database, $persistency = true)
	{
		...
		if($this->persistency)
		{
			$this->_connectionID = @mysql_pconnect($this->server, $this->user, $this->password);
		}
		else
		{
			$this->_connectionID = @mysql_connect($this->server, $this->user, $this->password);
		}
		if($this->_connectionID)
		{
			if($database != "")
	...
	function sql_close()
	{
		if($this->_connectionID)
		{
			...
			$result = @mysql_close($this->_connectionID);
			return $result;
		}
		else
		{
	...
	function sql_query($query = "", $transaction = FALSE)
	{
		...
		if($query != "")
		{
			...
			$this->query_result = @mysql_query($query, $this->_connectionID);
		}
		if($this->query_result)
		{
	...
	function sql_affectedrows()
	{
		if($this->_connectionID)
		{
			$result = @mysql_affected_rows($this->_connectionID);
			return $result;
		}
		else
		{
	...
	function sql_nextid(){
		if($this->_connectionID)
		{
			$result = @mysql_insert_id($this->_connectionID);
			return $result;
		}
		else
		{
	...
	{
		$result["message"] = @mysql_error($this->_connectionID);
		$result["code"] = @mysql_errno($this->_connectionID);
