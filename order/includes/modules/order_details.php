<?php
/* -----------------------------------------------------------------------------------------
   $Id: order_details.php,v 1.7 2003/12/30 09:02:31 oldpa   Exp $   
...
$module_smarty->assign('tpl_path','templates/'.CURRENT_TEMPLATE.'/');
  ...
  if ($customer_status_value->fields['customers_status_discount'] != 0) {
    $colspan++;
  ...
  echo '</tr>';
  echo '<tr>';
  echo '<td colspan="' . $colspan . '">' . twe_draw_separator() . '</td>';
  echo '</tr>';
...
// Delete box only for shopping cart
    if (strstr($PHP_SELF, FILENAME_SHOPPING_CART)) {
      echo '    <td align="center" valign="top">' . twe_draw_checkbox_field('cart_delete[]', $products[$i]['id']) . '</td>' . "\n";
    }

// Quantity box or information as an input box or text
    if (strstr($PHP_SELF, FILENAME_SHOPPING_CART)) {
      echo '    <td align="center" valign="top">' . twe_draw_input_field('cart_quantity[]', $products[$i]['quantity'], 'size="4"') . twe_draw_hidden_field('products_id[]', $products[$i]['id']) . '</td>' . "\n";
    } else {
...
// Model
    if ((PRODUCT_LIST_MODEL > 0) && strstr($PHP_SELF, FILENAME_SHOPPING_CART)) {
      echo '    <td valign="top" class="main"><a href="' . twe_href_link(FILENAME_PRODUCT_INFO, 'products_id=' . $products[$i]['id']) . '">' . $products[$i]['model'] . '</a></td>' . "\n";
    }
  
// Product name, with or without link
    if (strstr($PHP_SELF, FILENAME_SHOPPING_CART)) {
      echo '    <td valign="top" class="main"><a href="' . twe_href_link(FILENAME_PRODUCT_INFO, 'products_id=' . $products[$i]['id']) . '"><b>' . $products[$i]['name'] . '</b></a>';
    } else {
...
// Display marker if stock quantity insufficient
    if (!strstr($PHP_SELF, FILENAME_ACCOUNT_HISTORY_INFO)) {
      if (STOCK_CHECK == 'true') {
        echo $stock_check = twe_check_stock($products[$i]['id'], $products[$i]['quantity']);
...
// Display marker if attributes-stock quantity insufficient	
    if (!strstr($PHP_SELF, FILENAME_ACCOUNT_HISTORY_INFO)) {
      if (ATTRIBUTE_STOCK_CHECK == 'true') {
        ...
        echo $attribute_stock_check = twe_check_stock_attributes($products[$i][$option]['attributes_stock'], $products[$i]['quantity']);
...
// elari - changed CS V3.x
  if ($customer_status_value->fields['customers_status_discount'] != 0) {
  $max_product_discount = min($products[$i]['discount_allowed'] , $customer_status_value->fields['customers_status_discount']);
  echo $products[$i]['discount_allowed'] . $products[$i]['discount_allowed'] . $customer_status_value->fields['customers_status_discount'];
...
// elari End CS V3.x
  if (!strstr($PHP_SELF, FILENAME_ACCOUNT_HISTORY_INFO)) {
    echo '    <td align="right" valign="top" class="main"><b>'.twe_get_products_price($products[$i]['id'],$price_special=1,$quantity=$products[$i]['quantity']).'</b>' . "\n";
  } else {
    echo '    <td align="right" valign="top" class="main"><b>'.twe_get_products_price($products[$i]['id'],$price_special=1,$quantity=$products[$i]['quantity']) . '</b>' . "\n";
  }
      ...
      while (list($option, $value) = each($products[$i]['attributes'])) {
        if ($products[$i][$option]['options_values_price'] != 0) {
          if (!strstr($PHP_SELF, FILENAME_ACCOUNT_HISTORY_INFO)) {
            ...
            echo '<br><small><i>' . twe_get_products_attribute_price($products[$i][$option]['options_values_price'], $tax_class=$products[$i]['tax_class_id'],$price_special=1,$quantity=$products[$i]['quantity'],$prefix= $products[$i][$option]['price_prefix']) . '</i></small>';
          } else {
            echo '<br><small><i>' . twe_get_products_attribute_price($products[$i][$option]['options_values_price'], $tax_class=$products[$i]['tax_class_id'],$price_special=1,$quantity=$products[$i]['quantity'],$prefix= $products[$i][$option]['price_prefix']) . '</i></small>';
          }
        } else {
