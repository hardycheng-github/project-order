...
/* -----------------------------------------------------------------------------------------
  ...
  if (isset($_SESSION['customer_id'])) {
    $account = "select customers_firstname, customers_lastname, customers_email_address from " . TABLE_CUSTOMERS . " where customers_id = '" . (int)$_SESSION['customer_id'] . "'";
    $account_values = $db->Execute($account);
 ...
 if (isset($_GET['products_id'])) {
  ...
  $breadcrumb->add(NAVBAR_TITLE_TELL_A_FRIEND, twe_href_link(FILENAME_TELL_A_FRIEND, 'send_to=' . $_GET['send_to'] . '&products_id=' . $_GET['products_id']));
...
twe_redirect(FILENAME_DEFAULT);
    ...
    if (isset($_SESSION['customer_id'])) {
      $from_name = $account_values->fields['customers_firstname'] . ' ' . $account_values->fields['customers_lastname'];
      $from_email_address = $account_values->fields['customers_email_address'];
    } else {
    ...
    if (isset($_GET['action']) && ($_GET['action'] == 'process') && ($error == false)) {
      ...
      $mail_smarty->assign('PRODUCTS_LINK',twe_href_link(FILENAME_PRODUCT_INFO, 'products_id=' . $_GET['products_id']));
      ...
      $smarty->assign('BUTTON_CONTINUE','<a href="' . twe_href_link(FILENAME_PRODUCT_INFO, 'products_id=' . $_GET['products_id']) . '">' . twe_image_button('button_continue.gif', IMAGE_BUTTON_CONTINUE) . '</a>');
      ...
      if (isset($_SESSION['customer_id'])) {
        $your_name_prompt = $account_values->fields['customers_firstname'] . ' ' . $account_values->fields['customers_lastname'];
        $your_email_address_prompt = $account_values->fields['customers_email_address'];
      } else {
...
$smarty->assign('FORM_ACTION',twe_draw_form('email_friend', twe_href_link(FILENAME_TELL_A_FRIEND, 'action=process&products_id=' . $_GET['products_id'])) . twe_draw_hidden_field('products_name', $product_info->fields['products_name']));
...
$smarty->assign('BUTTON_BACK','<a href="' . twe_href_link(FILENAME_PRODUCT_INFO, 'products_id=' . $_GET['products_id']) . '">' . twe_image_button('button_back.gif', IMAGE_BUTTON_BACK) . '</a>');
