...
/* -----------------------------------------------------------------------------------------
  ...
  if (!isset($_SESSION['sendto'])) {
    (twe_href_link(FILENAME_CHECKOUT_PAYMENT, '', 'SSL'));
  }
...
include('send_order.php');
  ...
  //BILL
  require_once(DIR_WS_MODULES . 'twraec/twraec_order.php');
  
  $xml_value = twraec_pre_process($order, $insert_id, $date_purchased);
  $xml_content = buildXml($order, $xml_value);
  $tmp_file = "order-" . $xml_value['order_no'];
  $xml_file = $tmp_file.".xml";
  $res_file = $tmp_file.".res";
  $inv_file = $tmp_file.".inv";
  $dis_file = $tmp_file.".dis";
  $print_url = $print_cgi_url . "?order=" . urlencode($xml_value['order_no']) . "&domain=" . urlencode(DOMAIN_NAME);
  $xml_path = DIR_FS_ORDER_XML . $xml_file;
  $res_path = DIR_FS_ORDER_XML . $res_file;
  $inv_path = DIR_FS_ORDER_XML . $inv_file;
  $dis_path = DIR_FS_ORDER_XML . $dis_file;
  $xml_result = writeOrderFile($xml_content, $insert_id, $xml_path);
  $discountCreateSuccess = writeDiscountFile($insert_id, $dis_path);
  //$icer_res定義在configure_extra
  if(file_exists($icer_res)) copy($icer_res,$res_path);
  
  //取得發票並建立
  $invoice_result = makeInvoice($order, $insert_id, $inv_path);
  if( $invoice_result == 'writeInvoiceFile success' ){
	  
  } else if( $invoice_result == 'writeInvoiceFile fail' ){
	  
  } else if( $invoice_result == 'no invoice can use' ){
  ...
  //myEdit 訂單成立後之工作
  //echo "<script>alert('訂單成立!');</script>";
  //header("Location: " . $print_url);
  //exit(1);
	$ch = curl_init();
	curl_setopt($ch, CURLOPT_URL, $print_url);
	...
	curl_close($ch);
	twe_redirect(twe_href_link(FILENAME_PRINT_OVER.'?order='.$insert_id));
	exit(1);
?>