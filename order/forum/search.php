...
else if ( $search_keywords != '' || $search_author != '' || $search_id )
{
	...
	if ( $search_id == 'newposts' || $search_id == 'egosearch' || $search_id == 'unanswered' || $search_keywords != '' || $search_author != '' )
	{
		if ( $search_id == 'newposts' || $search_id == 'egosearch' || ( $search_author != '' && $search_keywords == '' )  )
		{
			...
			else if ( $search_id == 'egosearch' )
			{
				if ( $userdata['session_logged_in'] )
				{
					$sql = "SELECT post_id 
						FROM " . POSTS_TABLE . " 
						WHERE poster_id = " . $userdata['customers_id'];
				}
				else
				{
			...
			else
			{
				...
				if ( $row = $db->sql_fetchrow($result) )
				{
					do
					{
						$matching_userids .= ( ( $matching_userids != '' ) ? ', ' : '' ) . $row['customers_id'];
					}
		...
		else if ( $search_keywords != '' )
		{
			...
			$split_search = array();
			$split_search = ( !strstr($multibyte_charset, $lang['ENCODING']) ) ?  split_words(clean_words('search', stripslashes($search_keywords), $stopword_array, $synonym_array), 'search') : preg_split(' ', $search_keywords);	
				...
				for ($i = 0; $i < count($search_id_chunks); $i++)
				{
					...
					else
					{
						...
						if ( $search_author != '' )
						{
							...
							$where_sql .= " AND u.customers_id = p.poster_id AND u.username LIKE '$search_author' ";
						}
	...
	if ( $search_results != '' )
	{
		...
		{
			$sql = "SELECT pt.post_text, pt.bbcode_uid, pt.post_subject, p.*, f.forum_id, f.forum_name, t.*, u.username, u.customers_id, u.user_sig, u.user_sig_bbcode_uid  
		...
		{
			$sql = "SELECT t.*, f.forum_id, f.forum_name, u.username, u.customers_id, u2.username as user2, u2.customers_id as id2, p.post_username, p2.post_username AS post_username2, p2.post_time 
				...
				WHERE t.topic_id IN ($search_results) 
					...
					AND u2.customers_id = p2.poster_id";
		}
		...
		for($i = 0; $i < count($searchset); $i++)
		{
			...
			if ( $show_results == 'posts' )
			{
				...
				$poster = ( $searchset[$i]['customers_id'] != ANONYMOUS ) ? '<a href="' . append_sid("profile.$phpEx?mode=viewprofile&amp;" . POST_USERS_URL . "=" . $searchset[$i]['customers_id']) . '">' : '';
				$poster .= ( $searchset[$i]['customers_id'] != ANONYMOUS ) ? $searchset[$i]['username'] : ( ( $searchset[$i]['post_username'] != "" ) ? $searchset[$i]['post_username'] : $lang['Guest'] );
				$poster .= ( $searchset[$i]['customers_id'] != ANONYMOUS ) ? '</a>' : '';
			...
			else
			{
				...
				$topic_author = ( $searchset[$i]['customers_id'] != ANONYMOUS ) ? '<a href="' . append_sid("profile.$phpEx?mode=viewprofile&amp;" . POST_USERS_URL . '=' . $searchset[$i]['customers_id']) . '">' : '';
				$topic_author .= ( $searchset[$i]['customers_id'] != ANONYMOUS ) ? $searchset[$i]['username'] : ( ( $searchset[$i]['post_username'] != '' ) ? $searchset[$i]['post_username'] : $lang['Guest'] );

				$topic_author .= ( $searchset[$i]['customers_id'] != ANONYMOUS ) ? '</a>' : '';
