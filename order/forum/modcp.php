...
switch( $mode )
{
		...
		if ( $confirm )
		{
			...
			while ( $row = $db->sql_fetchrow($result) )
			{
				$count_sql[] = "UPDATE " . USERS_TABLE . " 
					...
					WHERE customers_id = " . $row['poster_id'];
			}
			$db->sql_freeresult($result);
			...
			if ( !empty($topic_id) )
			{
				$redirect_page = "viewforum.$phpEx?" . POST_FORUM_URL . "=$forum_id&amp;Twesid=" . $userdata['session_id'];
				$l_redirect = sprintf($lang['Click_return_forum'], '<a href="' . $redirect_page . '">', '</a>');
			}
			else
			{
				$redirect_page = "modcp.$phpEx?" . POST_FORUM_URL . "=$forum_id&amp;Twesid=" . $userdata['session_id'];
	...
	case 'move':
		...
		if ( $confirm )
		{
			...
			if ( !empty($topic_id) )
			{
				$redirect_page = "viewtopic.$phpEx?" . POST_TOPIC_URL . "=$topic_id&amp;Twesid=" . $userdata['session_id'];
			...
			$message = $message . '<br \><br \>' . sprintf($lang['Click_return_forum'], '<a href="' . "viewforum.$phpEx?" . POST_FORUM_URL . "=$old_forum_id&amp;Twesid=" . $userdata['session_id'] . '">', '</a>');
	...
	case 'lock':
		...
		$message = $message . '<br \><br \>' . sprintf($lang['Click_return_forum'], '<a href="' . "viewforum.$phpEx?" . POST_FORUM_URL . "=$forum_id&amp;Twesid=" . $userdata['session_id'] . '">', '</a>');
	...
	case 'split':
		...
		if ($post_id_sql != '')
		{
			...
			if ($row = $db->sql_fetchrow($result))
			{
				...
				do
				{
					$customers_id_sql .= (($customers_id_sql != '') ? ', ' : '') . intval($row['poster_id']);
					$post_id_sql .= (($post_id_sql != '') ? ', ' : '') . intval($row['post_id']);;
				}
				while ($row = $db->sql_fetchrow($result));

				$post_subject = trim(dohtmlspecialchars($HTTP_POST_VARS['subject']));
				...
				$sql = "UPDATE " . TOPICS_WATCH_TABLE . " 
					...
					WHERE topic_id = $topic_id 
						AND customers_id IN ($customers_id_sql)";
				if (!$db->sql_query($sql))
				{
				...
				$template->assign_vars(array(
					'META' => '<meta http-equiv="refresh" content="3;url=' . "viewtopic.$phpEx?" . POST_TOPIC_URL . "=$topic_id&amp;Twesid=" . $userdata['session_id'] . '">')
				);

				$message = $lang['Topic_split'] . '<br /><br />' . sprintf($lang['Click_return_topic'], '<a href="' . "viewtopic.$phpEx?" . POST_TOPIC_URL . "=$topic_id&amp;Twesid=" . $userdata['session_id'] . '">', '</a>');
				message_die(GENERAL_MESSAGE, $message);
			}
		}
		else
		{
			...
			$sql = "SELECT u.username, p.*, pt.post_text, pt.bbcode_uid, pt.post_subject, p.post_username
				...
				WHERE p.topic_id = $topic_id
					AND p.poster_id = u.customers_id
	...
	case 'ip':
		...
		$template->assign_vars(array(
			...
			'U_LOOKUP_IP' => "modcp.$phpEx?mode=ip&amp;" . POST_POST_URL . "=$post_id&amp;" . POST_TOPIC_URL . "=$topic_id&amp;rdns=$ip_this_post&amp;Twesid=" . $userdata['session_id'])
		);
		...
		if ( $row = $db->sql_fetchrow($result) )
		{
			...
			do
			{
				...
				$template->assign_block_vars('iprow', array(
					...
					'U_LOOKUP_IP' => "modcp.$phpEx?mode=ip&amp;" . POST_POST_URL . "=$post_id&amp;" . POST_TOPIC_URL . "=$topic_id&amp;rdns=" . $row['poster_ip'] . "&amp;Twesid=" . $userdata['session_id'])
				);
		...
		//
		$sql = "SELECT u.customers_id, u.username, COUNT(*) as postings 
			FROM " . USERS_TABLE ." u, " . POSTS_TABLE . " p 
			WHERE p.poster_id = u.customers_id 
				AND p.poster_ip = '" . $post_row['poster_ip'] . "'
			GROUP BY u.customers_id, u.username
			ORDER BY " . (( SQL_LAYER == 'msaccess' ) ? 'COUNT(*)' : 'postings' ) . " DESC";
		if ( !($result = $db->sql_query($sql)) )
		{
		...
		if ( $row = $db->sql_fetchrow($result) )
		{
			...
			do
			{
				...
				$template->assign_block_vars('userrow', array(
					...
					'U_PROFILE' => ($id == ANONYMOUS) ? "modcp.$phpEx?mode=ip&amp;" . POST_POST_URL . "=" . $post_id . "&amp;" . POST_TOPIC_URL . "=" . $topic_id . "&amp;Twesid=" . $userdata['session_id'] : append_sid("profile.$phpEx?mode=viewprofile&amp;" . POST_USERS_URL . "=$id"),
	...
	default:
		...
		$sql = "SELECT t.*, u.username, u.customers_id, p.post_time
			FROM " . TOPICS_TABLE . " t, " . USERS_TABLE . " u, " . POSTS_TABLE . " p
			WHERE t.forum_id = $forum_id
				AND t.topic_poster = u.customers_id
		...
		while ( $row = $db->sql_fetchrow($result) )
		{
			...
			$u_view_topic = "modcp.$phpEx?mode=split&amp;" . POST_TOPIC_URL . "=$topic_id&amp;Twesid=" . $userdata['session_id'];
		...
		$template->assign_vars(array(
			'PAGINATION' => generate_pagination("modcp.$phpEx?" . POST_FORUM_URL . "=$forum_id&amp;Twesid=" . $userdata['session_id'], $forum_topics, $board_config['topics_per_page'], $start),
