...
if ( isset($HTTP_POST_VARS['edit']) || isset($HTTP_POST_VARS['new']) )
{
	...
	{
		if ( $row['customers_id'] == $group_info['group_moderator'] ) 
		{
...
else if ( isset($HTTP_POST_VARS['group_update']) )
{
		...
		if (intval($row['auth_mod']) == 1)
		{
			...
			for ($i = 0; $i < count($rows); $i++)
			{
				...
				WHERE (a.auth_mod = 1) AND (g.group_id = a.group_id) AND (a.group_id = ug.group_id) AND (g.group_id = ug.group_id) 
					AND (ug.customers_id = " . intval($rows[$i]['customers_id']) . ") AND (ug.group_id <> " . $group_id . ")";
				if ( !($result = $db->sql_query($sql)) )
				{
				...
				if ($db->sql_numrows($result) == 0)
				{
					...
					WHERE user_level = " . MOD . " AND customers_id = " . intval($rows[$i]['customers_id']);
	...
	else
	{
		...
		$group_moderator = $this_userdata['customers_id'];
		...
		if( $mode == "editgroup" )
		{
			...
			if ( $group_info['group_moderator'] != $group_moderator )
			{
				...
				$sql = "SELECT customers_id 
					...
					WHERE customers_id = $group_moderator 
						AND group_id = $group_id";
				if ( !($result = $db->sql_query($sql)) )
				{
