...
elseif( isset($HTTP_GET_VARS['pane']) && $HTTP_GET_VARS['pane'] == 'right' )
{
	...
	//
	$sql = "SELECT u.customers_id, u.username, u.user_session_time, u.user_session_page, s.session_logged_in, s.session_ip, s.session_start 
		FROM " . USERS_TABLE . " u, " . SESSIONS_TABLE . " s
		WHERE s.session_logged_in = " . TRUE . " 
			AND u.customers_id = s.session_user_id 
			AND u.customers_id <> " . ANONYMOUS . " 
	...
	if( count($onlinerow_reg) )
	{
		...
		{
			if( !inarray($onlinerow_reg[$i]['customers_id'], $reg_userid_ary) )
			{
				$reg_userid_ary[] = $onlinerow_reg[$i]['customers_id'];
				...
				$template->assign_block_vars("reg_user_row", array(
					...
					"U_USER_PROFILE" => append_sid("admin_users.$phpEx?mode=edit&amp;" . POST_USERS_URL . "=" . $onlinerow_reg[$i]['customers_id']),
