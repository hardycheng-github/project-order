...
if ( $mode == 'edit' || $mode == 'save' && ( isset($HTTP_POST_VARS['username']) || isset($HTTP_GET_VARS[POST_USERS_URL]) || isset( $HTTP_POST_VARS[POST_USERS_URL]) ) )
{
	...
	{
		$customers_id = intval($HTTP_POST_VARS['id']);

		if (!($this_userdata = get_userdata($customers_id)))
		{
			message_die(GENERAL_MESSAGE, $lang['No_customers_id_specified'] );
		}
		...
		if( $HTTP_POST_VARS['deleteuser'] )
		{
			$sql = "SELECT g.group_id 
				...
				WHERE ug.customers_id = $customers_id 
			...
			$sql = "UPDATE " . TOPICS_TABLE . "
				...
				WHERE topic_poster = $customers_id";
			if( !$db->sql_query($sql) )
			{
			...
			$sql = "UPDATE " . VOTE_USERS_TABLE . "
				...
				WHERE vote_user_id = $customers_id";
			if( !$db->sql_query($sql) )
			{
			...
			$sql = "SELECT group_id
				FROM " . GROUPS_TABLE . "
				WHERE group_moderator = $customers_id";
			if( !($result = $db->sql_query($sql)) )
			{
			...
			if ( count($group_moderator) )
			{
				...
				$sql = "UPDATE " . GROUPS_TABLE . "
					SET group_moderator = " . $userdata['customers_id'] . "
...
//--------------------------------------------------------------------twe oldpa
			...
			if( !$db->sql_query($sql) )
			{
				message_die(GENERAL_ERROR, 'Could not delete ADDRESS_BOOK_TABLE', '', __LINE__, __FILE__, $sql);
			}
			$sql = "DELETE FROM " . TABLE_CUSTOMERS_INFO . "
				WHERE customers_info_id = $customers_id";
			if( !$db->sql_query($sql) )
			{
				message_die(GENERAL_ERROR, 'Could not delete CUSTOMERS_INFO_TABLE', '', __LINE__, __FILE__, $sql);
			}
			$sql = "DELETE FROM " . TABLE_CUSTOMERS_BASKET . "
				WHERE customers_id = $customers_id";
			if( !$db->sql_query($sql) )
			{
				message_die(GENERAL_ERROR, 'Could not delete TABLE_CUSTOMERS_BASKET', '', __LINE__, __FILE__, $sql);
			}
			...
			if( !$db->sql_query($sql) )
			{
				message_die(GENERAL_ERROR, 'Could not delete TABLE_CUSTOMERS_BASKET_ATTRIBUTES', '', __LINE__, __FILE__, $sql);
			}
			...
			if( !$db->sql_query($sql) )
			{
				message_die(GENERAL_ERROR, 'Could not delete TABLE_PRODUCTS_NOTIFICATIONS', '', __LINE__, __FILE__, $sql);
			}
			...
			if( !$db->sql_query($sql) )
			{
				message_die(GENERAL_ERROR, 'Could not delete TABLE_WHOS_ONLINE', '', __LINE__, __FILE__, $sql);
			}
	
			$sql = "DELETE FROM " . TABLE_CUSTOMERS_STATUS_HISTORY . "
				WHERE customers_id = $customers_id";
			if( !$db->sql_query($sql) )
			{
				message_die(GENERAL_ERROR, 'Could not delete TABLE_CUSTOMERS_STATUS_HISTORY', '', __LINE__, __FILE__, $sql);
			}
	
			$sql = "DELETE FROM " . TABLE_CUSTOMERS_IP . "
				WHERE customers_id = $customers_id";			
			if( !$db->sql_query($sql) )
			{
				message_die(GENERAL_ERROR, 'Could not delete TABLE_CUSTOMERS_IP', '', __LINE__, __FILE__, $sql);
			}
...
//-----------------------------------------------------------------------------------				
			...
			$sql = "SELECT privmsgs_id
				FROM " . PRIVMSGS_TABLE . "
				WHERE privmsgs_from_userid = $customers_id 
					OR privmsgs_to_userid = $customers_id";
			if ( !($result = $db->sql_query($sql)) )
			{
		...
		$username = ( !empty($HTTP_POST_VARS['username']) ) ? trim(strip_tags(dohtmlspecialchars($HTTP_POST_VARS['username']))) : '';
		$email = ( !empty($HTTP_POST_VARS['email']) ) ? trim(strip_tags(dohtmlspecialchars( $HTTP_POST_VARS['email'] ) )) : '';

		$password = ( !empty($HTTP_POST_VARS['password']) ) ? trim(strip_tags(dohtmlspecialchars( $HTTP_POST_VARS['password'] ) )) : '';
		$password_confirm = ( !empty($HTTP_POST_VARS['password_confirm']) ) ? trim(strip_tags(dohtmlspecialchars( $HTTP_POST_VARS['password_confirm'] ) )) : '';
		...
		if( isset( $HTTP_POST_VARS['avatargallery'] ) || isset( $HTTP_POST_VARS['submitavatar'] ) || isset( $HTTP_POST_VARS['cancelavatar'] ) )
		{
			...
			$aim = dohtmlspecialchars(stripslashes($aim));
			$msn = dohtmlspecialchars(stripslashes($msn));
			$yim = dohtmlspecialchars(stripslashes($yim));

			$website = dohtmlspecialchars(stripslashes($website));
			$location = dohtmlspecialchars(stripslashes($location));
			$occupation = dohtmlspecialchars(stripslashes($occupation));
			$interests = dohtmlspecialchars(stripslashes($interests));
			$signature = dohtmlspecialchars(stripslashes($signature));

			$user_lang = stripslashes($user_lang);
			$user_dateformat = dohtmlspecialchars(stripslashes($user_dateformat));
	...
	if( isset( $HTTP_POST_VARS['submit'] ) )
	{
		...
		if( !empty($password) && !empty($password_confirm) )
		{
			...
			else
			{
				...
				$passwd_sql = "customers_password = '$password', ";
			}
		}
				...
				if( file_exists(@phpbb_realpath($user_avatar_loc)) && preg_match("/.jpg$|.gif$|.png$/", $user_avatar_name) )
				{
					if( $user_avatar_size <= $board_config['avatar_filesize'] && $user_avatar_size > 0)
					{
						...
						if( !$error )
						{
							...
							{
								$customers_id = $this_userdata['customers_id'];

								$avatar_filename = $customers_id . $imgtype;
					...
					if( $fsock )
					{
						...
						if( preg_match("/Content-Length\: ([0-9]+)[^\/ ][\s]+/i", $avatar_data, $file_data1) && preg_match("/Content-Type\: image\/[x\-]*([a-z]+)[\s]+/i", $avatar_data, $file_data2) )
						{
							...
							if( !$error && $file_size > 0 && $file_size < $board_config['avatar_filesize'] )
							{
								...
								$tmp_filename = tempnam ("/tmp", $this_userdata['customers_id'] . "-");
		...
		if( !$error )
		{
			$sql = "UPDATE " . USERS_TABLE . "
				SET " . $username_sql . $passwd_sql . "customers_email_address = '" . str_replace("\'", "''", $email) . "', user_icq = '" . str_replace("\'", "''", $icq) . "', user_website = '" . str_replace("\'", "''", $website) . "', user_occ = '" . str_replace("\'", "''", $occupation) . "', user_from = '" . str_replace("\'", "''", $location) . "', user_interests = '" . str_replace("\'", "''", $interests) . "', user_sig = '" . str_replace("\'", "''", $signature) . "', user_viewemail = $viewemail, user_aim = '" . str_replace("\'", "''", $aim) . "', user_yim = '" . str_replace("\'", "''", $yim) . "', user_msnm = '" . str_replace("\'", "''", $msn) . "', user_attachsig = $attachsig, user_sig_bbcode_uid = '$signature_bbcode_uid', user_allowsmile = $allowsmilies, user_allowhtml = $allowhtml, user_allowavatar = $user_allowavatar, user_allowbbcode = $allowbbcode, user_allow_viewonline = $allowviewonline, user_notify = $notifyreply, user_allow_pm = $user_allowpm, user_notify_pm = $notifypm, user_popup_pm = $popuppm, user_lang = '" . str_replace("\'", "''", $user_lang) . "', user_style = $user_style, user_timezone = $user_timezone, user_dateformat = '" . str_replace("\'", "''", $user_dateformat) . "', user_active = $user_status, user_rank = $user_rank" . $avatar_sql . "
				WHERE customers_id = $customers_id";
			...
			if( $result = $db->sql_query($sql) )
			{
				...
				if (!$user_status)
				{
					$sql = "DELETE FROM " . SESSIONS_TABLE . " 
						WHERE session_user_id = " . $customers_id;
		...
		else
		{
			...
			$username = dohtmlspecialchars(stripslashes($username));
			$email = stripslashes($email);
			$password = '';
			$password_confirm = '';

			$icq = stripslashes($icq);
			$aim = dohtmlspecialchars(str_replace('+', ' ', stripslashes($aim)));
			$msn = dohtmlspecialchars(stripslashes($msn));
			$yim = dohtmlspecialchars(stripslashes($yim));
	...
	else if( !isset( $HTTP_POST_VARS['submit'] ) && $mode != 'save' && !isset( $HTTP_POST_VARS['avatargallery'] ) && !isset( $HTTP_POST_VARS['submitavatar'] ) && !isset( $HTTP_POST_VARS['cancelavatar'] ) )
	{
		...
		{
			$customers_id = ( isset( $HTTP_POST_VARS[POST_USERS_URL]) ) ? intval( $HTTP_POST_VARS[POST_USERS_URL]) : intval( $HTTP_GET_VARS[POST_USERS_URL]);
			$this_userdata = get_userdata($customers_id);
			if( !$this_userdata )
			{
				message_die(GENERAL_MESSAGE, $lang['No_customers_id_specified'] );
			}
		...
		$email = $this_userdata['customers_email_address'];
		$password = '';
		$password_confirm = '';

		$icq = $this_userdata['user_icq'];
		$aim = dohtmlspecialchars(str_replace('+', ' ', $this_userdata['user_aim'] ));
		$msn = dohtmlspecialchars($this_userdata['user_msnm']);
		$yim = dohtmlspecialchars($this_userdata['user_yim']);

		$website = dohtmlspecialchars($this_userdata['user_website']);
		$location = dohtmlspecialchars($this_userdata['user_from']);
		$occupation = dohtmlspecialchars($this_userdata['user_occ']);
		$interests = dohtmlspecialchars($this_userdata['user_interests']);
		...
		$user_dateformat = dohtmlspecialchars($this_userdata['user_dateformat']);
	...
	if( isset($HTTP_POST_VARS['avatargallery']) && !$error )
	{
		if( !$error )
		{
			...
			{
				$category = dohtmlspecialchars($HTTP_POST_VARS['avatarcategory']);
			}
			else
			{
			...
			$s_hidden_fields .= '<input type="hidden" name="id" value="' . $customers_id . '" />';
	...
	else
	{
		...
		$s_hidden_fields .= '<input type="hidden" name="id" value="' . $this_userdata['customers_id'] . '" />';
