...
function get_table_def_postgresql($table, $crlf)
{
	...
	while ($row = $db->sql_fetchrow($result))
	{
		...
		if (preg_match('/numeric/', $row['type']))
		{
			$schema_create .= '(';
...
function get_table_content_postgresql($table, $handler)
{
	...
	while($row = $db->sql_fetchrow($result))
	{
		...
		for($i = 0; $i < $i_num_fields; $i++)
		{
			...
			if (preg_match("/char|text|bool/", $aryType[$i]))
			{
				$strQuote = "'";
				$strEmpty = "";
				...
				if($sql_query != "")
				{
					...
					for($i = 0; $i < $sql_count; $i++)
					{
						...
						if(!empty($sql) and $sql[0] != "#")
						{
							...
							if(!$result && ( !(SQL_LAYER == 'postgresql' && preg_match("/drop table/", $sql) ) ) )
							{
