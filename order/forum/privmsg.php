...
if ( isset($HTTP_POST_VARS['folder']) || isset($HTTP_GET_VARS['folder']) )
{
	...
	$folder = dohtmlspecialchars($folder);
...
else if ( $mode == 'read' )
{
	...
	switch( $folder )
	{
		case 'inbox':
			...
			$pm_sql_user = "AND pm.privmsgs_to_userid = " . $userdata['customers_id'] . " 
		...
		case 'outbox':
			...
			$pm_sql_user = "AND pm.privmsgs_from_userid =  " . $userdata['customers_id'] . " 
		...
		case 'savebox':
			...
			$pm_sql_user = "AND ( ( pm.privmsgs_to_userid = " . $userdata['customers_id'] . "
				...
				OR ( pm.privmsgs_from_userid = " . $userdata['customers_id'] . "
	...
	//
	$sql = "SELECT u.username AS username_1, u.customers_id AS customers_id_1, u2.username AS username_2, u2.customers_id AS customers_id_2, u.user_sig_bbcode_uid, u.user_posts, u.user_from, u.user_website, u.customers_email_address, u.user_icq, u.user_aim, u.user_yim, u.user_regdate, u.user_msnm, u.user_viewemail, u.user_rank, u.user_sig, u.user_avatar, pm.*, pmt.privmsgs_bbcode_uid, pmt.privmsgs_text
		FROM " . PRIVMSGS_TABLE . " pm, " . PRIVMSGS_TEXT_TABLE . " pmt, " . USERS_TABLE . " u, " . USERS_TABLE . " u2 
		WHERE pm.privmsgs_id = $privmsgs_id
			AND pmt.privmsgs_text_id = pm.privmsgs_id 
			$pm_sql_user 
			AND u.customers_id = pm.privmsgs_from_userid 
			AND u2.customers_id = pm.privmsgs_to_userid";
	...
	$customers_id_from = $privmsg['customers_id_1'];
	$username_to = $privmsg['username_2'];
	$customers_id_to = $privmsg['customers_id_2'];

	$post_date = create_date($board_config['default_dateformat'], $privmsg['privmsgs_date'], $board_config['board_timezone']);

	$temp_url = append_sid("profile.$phpEx?mode=viewprofile&amp;" . POST_USERS_URL . '=' . $customers_id_from);
	$profile_img = '<a href="' . $temp_url . '"><img src="' . $images['icon_profile'] . '" alt="' . $lang['Read_profile'] . '" title="' . $lang['Read_profile'] . '" border="0" /></a>';
	$profile = '<a href="' . $temp_url . '">' . $lang['Read_profile'] . '</a>';

	$temp_url = append_sid("privmsg.$phpEx?mode=post&amp;" . POST_USERS_URL . "=$customers_id_from");
	$pm_img = '<a href="' . $temp_url . '"><img src="' . $images['icon_pm'] . '" alt="' . $lang['Send_private_message'] . '" title="' . $lang['Send_private_message'] . '" border="0" /></a>';
	$pm = '<a href="' . $temp_url . '">' . $lang['Send_private_message'] . '</a>';

	if ( !empty($privmsg['user_viewemail']) || $userdata['user_level'] == ADMIN )
	{
		$email_uri = ( $board_config['board_email_form'] ) ? append_sid("profile.$phpEx?mode=email&amp;" . POST_USERS_URL .'=' . $customers_id_from) : 'mailto:' . $privmsg['customers_email_address'];
	...
	{
		$user_sig = ( $privmsg['privmsgs_from_userid'] == $userdata['customers_id'] ) ? $userdata['user_sig'] : $privmsg['user_sig'];
	}
	else
	{
		$user_sig = '';
	...
	$user_sig_bbcode_uid = ( $privmsg['privmsgs_from_userid'] == $userdata['customers_id'] ) ? $userdata['user_sig_bbcode_uid'] : $privmsg['user_sig_bbcode_uid'];
...
else if ( ( $delete && $mark_list ) || $delete_all )
{
	...
	else if ( $confirm )
	{
		...
		{
			case 'inbox':
				$delete_type = "privmsgs_to_userid = " . $userdata['customers_id'] . " AND (
				privmsgs_type = " . PRIVMSGS_READ_MAIL . " OR privmsgs_type = " . PRIVMSGS_NEW_MAIL . " OR privmsgs_type = " . PRIVMSGS_UNREAD_MAIL . " )";
				break;

			case 'outbox':
				$delete_type = "privmsgs_from_userid = " . $userdata['customers_id'] . " AND ( privmsgs_type = " . PRIVMSGS_NEW_MAIL . " OR privmsgs_type = " . PRIVMSGS_UNREAD_MAIL . " )";
				break;

			case 'sentbox':
				$delete_type = "privmsgs_from_userid = " . $userdata['customers_id'] . " AND privmsgs_type = " . PRIVMSGS_SENT_MAIL;
				break;

			case 'savebox':
				$delete_type = "( ( privmsgs_from_userid = " . $userdata['customers_id'] . " 
					AND privmsgs_type = " . PRIVMSGS_SAVED_OUT_MAIL . " ) 
				OR ( privmsgs_to_userid = " . $userdata['customers_id'] . " 
		...
		if ( count($mark_list) )
		{
			...
			if ($folder == 'inbox' || $folder == 'outbox')
			{
				...
				{
					case 'inbox':
						$sql = "privmsgs_to_userid = " . $userdata['customers_id'];
						break;
					case 'outbox':
						$sql = "privmsgs_from_userid = " . $userdata['customers_id'];
						break;
				}
				...
				if ( $row = $db->sql_fetchrow($result))
				{
					...
					if (sizeof($update_users))
					{
						...
						{
							while (list($customers_id, $dec) = each($users))
							{
								$update_list[$type][$dec][] = $customers_id;
							}
						}
						unset($update_users);
						...
						while (list($type, $dec_ary) = each($update_list))
						{
							...
							{
								$customers_ids = implode(', ', $user_ary);

								$sql = "UPDATE " . USERS_TABLE . " 
									SET $type = $type - $dec 
									WHERE customers_id IN ($customers_ids)";
								if ( !$db->sql_query($sql) )
								{
			...
			{
				case 'inbox':
					$delete_sql .= "privmsgs_to_userid = " . $userdata['customers_id'] . " AND (
						privmsgs_type = " . PRIVMSGS_READ_MAIL . " OR privmsgs_type = " . PRIVMSGS_NEW_MAIL . " OR privmsgs_type = " . PRIVMSGS_UNREAD_MAIL . " )";
					break;

				case 'outbox':
					$delete_sql .= "privmsgs_from_userid = " . $userdata['customers_id'] . " AND ( 
						privmsgs_type = " . PRIVMSGS_NEW_MAIL . " OR privmsgs_type = " . PRIVMSGS_UNREAD_MAIL . " )";
					break;

				case 'sentbox':
					$delete_sql .= "privmsgs_from_userid = " . $userdata['customers_id'] . " AND privmsgs_type = " . PRIVMSGS_SENT_MAIL;
					break;

				case 'savebox':
					$delete_sql .= "( ( privmsgs_from_userid = " . $userdata['customers_id'] . " 
						AND privmsgs_type = " . PRIVMSGS_SAVED_OUT_MAIL . " ) 
					OR ( privmsgs_to_userid = " . $userdata['customers_id'] . " 
						AND privmsgs_type = " . PRIVMSGS_SAVED_IN_MAIL . " ) )";
					break;
...
else if ( $save && $mark_list && $folder != 'savebox' && $folder != 'outbox' )
{
	...
	if (sizeof($mark_list))
	{
		...
		$sql = "SELECT COUNT(privmsgs_id) AS savebox_items, MIN(privmsgs_date) AS oldest_post_time 
			FROM " . PRIVMSGS_TABLE . " 
			WHERE ( ( privmsgs_to_userid = " . $userdata['customers_id'] . " 
					AND privmsgs_type = " . PRIVMSGS_SAVED_IN_MAIL . " )
				OR ( privmsgs_from_userid = " . $userdata['customers_id'] . " 
					AND privmsgs_type = " . PRIVMSGS_SAVED_OUT_MAIL . ") )";
		if ( !($result = $db->sql_query($sql)) )
		{
			message_die(GENERAL_ERROR, 'Could not obtain sent message info for sendee', '', __LINE__, __FILE__, $sql);
		...
		switch ($folder)
		{
			...
			case 'outbox':
				$saved_sql .= " SET privmsgs_type = " . PRIVMSGS_SAVED_OUT_MAIL . " 
					WHERE privmsgs_from_userid = " . $userdata['customers_id'] . " 
...
else if ( $submit || $refresh || $mode != '' )
{
	if ( !$userdata['session_logged_in'] )
	{
		$customers_id = ( isset($HTTP_GET_VARS[POST_USERS_URL]) ) ? '&' . POST_USERS_URL . '=' . intval($HTTP_GET_VARS[POST_USERS_URL]) : '';
		redirect(append_sid("login.$phpEx?redirect=privmsg.$phpEx&folder=$folder&mode=$mode" . $customers_id, true));
	}
	...
	if ( $submit )
	{
		if ( !empty($HTTP_POST_VARS['username']) )
		{
			...
			$sql = "SELECT customers_id, user_notify_pm, customers_email_address, user_lang, user_active 
				FROM " . USERS_TABLE . "
				WHERE username = '" . str_replace("\'", "''", $to_username) . "'
					AND customers_id <> " . ANONYMOUS;
			if ( !($result = $db->sql_query($sql)) )
			{
				$error = TRUE;
	...
	if ( $submit && !$error )
	{
		...
		if ( $mode != 'edit' )
		{
			...
			$sql = "SELECT COUNT(privmsgs_id) AS inbox_items, MIN(privmsgs_date) AS oldest_post_time 
				...
				WHERE ( privmsgs_type = " . PRIVMSGS_NEW_MAIL . " 
						...
						OR privmsgs_type = " . PRIVMSGS_UNREAD_MAIL . " ) 
					AND privmsgs_to_userid = " . $to_userdata['customers_id'];
			if ( !($result = $db->sql_query($sql)) )
			{
			...
			$sql_info = "INSERT INTO " . PRIVMSGS_TABLE . " (privmsgs_type, privmsgs_subject, privmsgs_from_userid, privmsgs_to_userid, privmsgs_date, privmsgs_ip, privmsgs_enable_html, privmsgs_enable_bbcode, privmsgs_enable_smilies, privmsgs_attach_sig)
				VALUES (" . PRIVMSGS_NEW_MAIL . ", '" . str_replace("\'", "''", $privmsg_subject) . "', " . $userdata['customers_id'] . ", " . $to_userdata['customers_id'] . ", $msg_time, '$user_ip', $html_on, $bbcode_on, $smilies_on, $attach_sig)";
		}
		else
		{
			$sql_info = "UPDATE " . PRIVMSGS_TABLE . "
				SET privmsgs_type = " . PRIVMSGS_NEW_MAIL . ", privmsgs_subject = '" . str_replace("\'", "''", $privmsg_subject) . "', privmsgs_from_userid = " . $userdata['customers_id'] . ", privmsgs_to_userid = " . $to_userdata['customers_id'] . ", privmsgs_date = $msg_time, privmsgs_ip = '$user_ip', privmsgs_enable_html = $html_on, privmsgs_enable_bbcode = $bbcode_on, privmsgs_enable_smilies = $smilies_on, privmsgs_attach_sig = $attach_sig 
		...
		if ( $mode != 'edit' )
		{
			...
			$sql = "UPDATE " . USERS_TABLE . "
				...
				WHERE customers_id = " . $to_userdata['customers_id']; 
			if ( !$status = $db->sql_query($sql) )
			{
				message_die(GENERAL_ERROR, 'Could not update private message new/read status for user', '', __LINE__, __FILE__, $sql);
			}

			if ( $to_userdata['user_notify_pm'] && !empty($to_userdata['customers_email_address']) && $to_userdata['user_active'] )
			{
				...
				$emailer->email_address($to_userdata['customers_email_address']);
	...
	else if ( $preview || $refresh || $error )
	{
		...
		//
		$to_username = (isset($HTTP_POST_VARS['username']) ) ? trim(dohtmlspecialchars(stripslashes($HTTP_POST_VARS['username']))) : '';
		...
		else if ( $mode == 'edit' )
		{
			...
			$sql = "SELECT u.customers_id, u.user_sig 
			...
			{
				if ( $userdata['customers_id'] != $postrow['customers_id'] )
				{
	...
	else 
	{
		...
		{
			$customers_id = intval($HTTP_GET_VARS[POST_USERS_URL]);
		...
		if ( $mode == 'edit' )
		{
			$sql = "SELECT pm.*, pmt.privmsgs_bbcode_uid, pmt.privmsgs_text, u.username, u.customers_id, u.user_sig 
				FROM " . PRIVMSGS_TABLE . " pm, " . PRIVMSGS_TEXT_TABLE . " pmt, " . USERS_TABLE . " u
				WHERE pm.privmsgs_id = $privmsg_id
					AND pmt.privmsgs_text_id = pm.privmsgs_id
					AND pm.privmsgs_from_userid = " . $userdata['customers_id'] . "
					AND ( pm.privmsgs_type = " . PRIVMSGS_NEW_MAIL . " 
						OR pm.privmsgs_type = " . PRIVMSGS_UNREAD_MAIL . " ) 
					AND u.customers_id = pm.privmsgs_to_userid";
			if ( !($result = $db->sql_query($sql)) )
			{
			...
			$to_userid = $privmsg['customers_id'];

		}
		else if ( $mode == 'reply' || $mode == 'quote' )
		{

			$sql = "SELECT pm.privmsgs_subject, pm.privmsgs_date, pmt.privmsgs_bbcode_uid, pmt.privmsgs_text, u.username, u.customers_id
				FROM " . PRIVMSGS_TABLE . " pm, " . PRIVMSGS_TEXT_TABLE . " pmt, " . USERS_TABLE . " u
				WHERE pm.privmsgs_id = $privmsg_id
					AND pmt.privmsgs_text_id = pm.privmsgs_id
					AND pm.privmsgs_to_userid = " . $userdata['customers_id'] . "
					AND u.customers_id = pm.privmsgs_from_userid";
			if ( !($result = $db->sql_query($sql)) )
			{
				message_die(GENERAL_ERROR, 'Could not obtain private message for editing', '', __LINE__, __FILE__, $sql);
...
$sql = "UPDATE " . PRIVMSGS_TABLE . "
	...
	WHERE privmsgs_type = " . PRIVMSGS_NEW_MAIL . " 
		AND privmsgs_to_userid = " . $userdata['customers_id'];
if ( !$db->sql_query($sql) )
{
...
$sql_tot = "SELECT COUNT(privmsgs_id) AS total 
	FROM " . PRIVMSGS_TABLE . " ";
$sql = "SELECT pm.privmsgs_type, pm.privmsgs_id, pm.privmsgs_date, pm.privmsgs_subject, u.customers_id, u.username 
	FROM " . PRIVMSGS_TABLE . " pm, " . USERS_TABLE . " u ";
switch( $folder )
{
	case 'inbox':
		$sql_tot .= "WHERE privmsgs_to_userid = " . $userdata['customers_id'] . "
			AND ( privmsgs_type =  " . PRIVMSGS_NEW_MAIL . "
				OR privmsgs_type = " . PRIVMSGS_READ_MAIL . " 
				OR privmsgs_type = " . PRIVMSGS_UNREAD_MAIL . " )";

		$sql .= "WHERE pm.privmsgs_to_userid = " . $userdata['customers_id'] . "
			AND u.customers_id = pm.privmsgs_from_userid
			AND ( pm.privmsgs_type =  " . PRIVMSGS_NEW_MAIL . "
	...
	case 'outbox':
		$sql_tot .= "WHERE privmsgs_from_userid = " . $userdata['customers_id'] . "
			AND ( privmsgs_type =  " . PRIVMSGS_NEW_MAIL . "
				OR privmsgs_type = " . PRIVMSGS_UNREAD_MAIL . " )";

		$sql .= "WHERE pm.privmsgs_from_userid = " . $userdata['customers_id'] . "
			AND u.customers_id = pm.privmsgs_to_userid
			AND ( pm.privmsgs_type =  " . PRIVMSGS_NEW_MAIL . "
				OR privmsgs_type = " . PRIVMSGS_UNREAD_MAIL . " )";
		break;
	...
	case 'savebox':
		...
		$sql .= "WHERE u.customers_id = pm.privmsgs_from_userid 
			AND ( ( pm.privmsgs_to_userid = " . $userdata['customers_id'] . " 
				AND pm.privmsgs_type = " . PRIVMSGS_SAVED_IN_MAIL . " ) 
			OR ( pm.privmsgs_from_userid = " . $userdata['customers_id'] . " 
				AND pm.privmsgs_type = " . PRIVMSGS_SAVED_OUT_MAIL . " ) )";
