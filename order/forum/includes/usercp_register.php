	...
	$mode == 'register' )
{
	...
	if ( $mode == 'editprofile' )
	{
		$customers_id = intval($HTTP_POST_VARS['customers_id']);
		$current_email = trim(dohtmlspecialchars($HTTP_POST_VARS['current_email']));
	}
	...
	// to use dohtmlspecialchars ... be prepared to be moaned at.
	while( list($var, $param) = @each($strip_var_list) )
	{
		if ( !empty($HTTP_POST_VARS[$param]) )
		{
			$$var = trim(dohtmlspecialchars($HTTP_POST_VARS[$param]));
		}
	}
	...
	if ( !empty($HTTP_POST_VARS['language']) )
	{
		...
		{
			$user_lang = dohtmlspecialchars($HTTP_POST_VARS['language']);
		}
		else
		{
			$error = true;
	...
	$user_dateformat = ( !empty($HTTP_POST_VARS['dateformat']) ) ? trim(dohtmlspecialchars($HTTP_POST_VARS['dateformat'])) : $board_config['default_dateformat'];

	$user_avatar_local = ( isset($HTTP_POST_VARS['avatarselect']) && !empty($HTTP_POST_VARS['submitavatar']) && $board_config['allow_avatar_local'] ) ? dohtmlspecialchars($HTTP_POST_VARS['avatarselect']) : ( ( isset($HTTP_POST_VARS['avatarlocal'])  ) ? dohtmlspecialchars($HTTP_POST_VARS['avatarlocal']) : '' );

	$user_avatar_remoteurl = ( !empty($HTTP_POST_VARS['avatarremoteurl']) ) ? trim(dohtmlspecialchars($HTTP_POST_VARS['avatarremoteurl'])) : '';
	...
	if ( (isset($HTTP_POST_VARS['avatargallery']) || isset($HTTP_POST_VARS['submitavatar']) || isset($HTTP_POST_VARS['cancelavatar'])) && (!isset($HTTP_POST_VARS['submit'])) )
	{
		...
		$cur_password = dohtmlspecialchars(stripslashes($cur_password));
		$new_password = dohtmlspecialchars(stripslashes($new_password));
		$password_confirm = dohtmlspecialchars(stripslashes($password_confirm));
...
if ( isset($HTTP_POST_VARS['submit']) )
{
	...
	if ( $mode == 'editprofile' )
	{
		if ( $customers_id != $userdata['customers_id'] )
		{
			$error = TRUE;
	...
	if ($board_config['enable_confirm'] && $mode == 'register')
	{
		...
		{
			$confirm_id = dohtmlspecialchars($HTTP_POST_VARS['confirm_id']);
	...
	if ( !empty($new_password) && !empty($password_confirm) )
	{
		...
		else
		{
			if ( $mode == 'editprofile' )
			{
				...
				if ( !($result = $db->sql_query($sql)) )
				{
					message_die(GENERAL_ERROR, 'Could not obtain customers_password information', '', __LINE__, __FILE__, $sql);
				}

				$row = $db->sql_fetchrow($result);

				if ( $row['customers_password'] != md5($cur_password) )
				{
					$error = TRUE;
			...
			if ( !$error )
			{
				...
				$passwd_sql = "customers_password = '$new_password', ";
			}
		}
	}
	...
	//
	if ( $email != $userdata['customers_email_address'] || $mode == 'register' )
	{
	...
	if ( !$error )
	{
		...
		if ( $mode == 'editprofile' )
		{
			if ( $email != $userdata['customers_email_address'] && $board_config['require_activation'] != USER_ACTIVATION_NONE && $userdata['user_level'] != ADMIN )
			{
				$user_active = 0;
		...
		{
			$sql = "SELECT MAX(customers_id) AS total
				FROM " . USERS_TABLE;
			if ( !($result = $db->sql_query($sql)) )
			{
				message_die(GENERAL_ERROR, 'Could not obtain next customers_id information', '', __LINE__, __FILE__, $sql);
			}
			...
			$customers_id = $row['total'] + 1;

			//
			// Get current date
			//
			$sql = "INSERT INTO " . USERS_TABLE . "	(customers_id, username, user_regdate, customers_password, customers_email_address, user_icq, user_website, user_occ, user_from, user_interests, user_sig, user_sig_bbcode_uid, user_avatar, user_avatar_type, user_viewemail, user_aim, user_yim, user_msnm, user_attachsig, user_allowsmile, user_allowhtml, user_allowbbcode, user_allow_viewonline, user_notify, user_notify_pm, user_popup_pm, user_timezone, user_dateformat, user_lang, user_style, user_level, user_allow_pm, user_active, user_actkey)
				VALUES ($customers_id, '" . str_replace("\'", "''", $username) . "', " . time() . ", '" . str_replace("\'", "''", $new_password) . "', '" . str_replace("\'", "''", $email) . "', '" . str_replace("\'", "''", $icq) . "', '" . str_replace("\'", "''", $website) . "', '" . str_replace("\'", "''", $occupation) . "', '" . str_replace("\'", "''", $location) . "', '" . str_replace("\'", "''", $interests) . "', '" . str_replace("\'", "''", $signature) . "', '$signature_bbcode_uid', $avatar_sql, $viewemail, '" . str_replace("\'", "''", str_replace(' ', '+', $aim)) . "', '" . str_replace("\'", "''", $yim) . "', '" . str_replace("\'", "''", $msn) . "', $attachsig, $allowsmilies, $allowhtml, $allowbbcode, $allowviewonline, $notifyreply, $notifypm, $popup_pm, $user_timezone, '" . str_replace("\'", "''", $user_dateformat) . "', '" . str_replace("\'", "''", $user_lang) . "', $user_style, 0, 1, ";
			...
			$sql = "INSERT INTO " . USER_GROUP_TABLE . " (customers_id, group_id, user_pending)
				VALUES ($customers_id, $group_id, 0)";
			...
			{
				$sql = "SELECT customers_email_address, user_lang 
					FROM " . USERS_TABLE . "
				...
				while ($row = $db->sql_fetchrow($result))
				{
					...
					$emailer->email_address(trim($row['customers_email_address']));
...
if( isset($HTTP_POST_VARS['avatargallery']) && !$error )
{
	...
	$avatar_category = ( !empty($HTTP_POST_VARS['avatarcategory']) ) ? dohtmlspecialchars($HTTP_POST_VARS['avatarcategory']) : '';
	...
	display_avatar_gallery($mode, $avatar_category, $customers_id, $email, $current_email, $coppa, $username, $email, &$new_password, &$cur_password, $password_confirm, $icq, $aim, $msn, $yim, $website, $location, $occupation, $interests, $signature, $viewemail, $notifypm, $popup_pm, $notifyreply, $attachsig, $allowhtml, $allowbbcode, $allowsmilies, $allowviewonline, $user_style, $user_lang, $user_timezone, $user_dateformat, $userdata['session_id']);
}
else
{
	...
	if( $mode == 'editprofile' )
	{
		$s_hidden_fields .= '<input type="hidden" name="customers_id" value="' . $userdata['customers_id'] . '" />';
		//
		// Send the users current email address. If they change it, and account activation is turned on
		// the user account will be disabled and the user will have to reactivate their account.
		//
		$s_hidden_fields .= '<input type="hidden" name="current_email" value="' . $userdata['customers_email_address'] . '" />';
	}
