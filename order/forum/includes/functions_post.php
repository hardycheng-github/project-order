...
function prepare_post(&$mode, &$post_data, &$bbcode_on, &$html_on, &$smilies_on, &$error_msg, &$username, &$bbcode_uid, &$subject, &$message, &$poll_title, &$poll_options, &$poll_length)
{
	...
	if (!empty($subject))
	{
		$subject = dohtmlspecialchars(trim($subject));
	}
	...
	if ($mode == 'newtopic' || ($mode == 'editpost' && $post_data['first_post']))
	{
		...
		if (!empty($poll_title))
		{
			$poll_title = dohtmlspecialchars(trim($poll_title));
		}
		...
		if(!empty($poll_options))
		{
			...
			while(list($option_id, $option_text) = @each($poll_options))
			{
				...
				{
					$temp_option_text[$option_id] = dohtmlspecialchars($option_text);
				}
			}
...
function submit_post($mode, &$post_data, &$message, &$meta, &$forum_id, &$topic_id, &$post_id, &$poll_id, &$topic_type, &$bbcode_on, &$html_on, &$smilies_on, &$attach_sig, &$bbcode_uid, &$post_username, &$post_subject, &$post_message, &$poll_title, &$poll_options, &$poll_length)
{
	...
	if ($mode == 'newtopic' || $mode == 'reply' || $mode == 'editpost') 
	{
		...
		// Flood control
		//
		$where_sql = ($userdata['customers_id'] == ANONYMOUS) ? "poster_ip = '$user_ip'" : 'poster_id = ' . $userdata['customers_id'];
	...
	if ($mode == 'newtopic' || ($mode == 'editpost' && $post_data['first_post']))
	{
		...
		$sql  = ($mode != "editpost") ? "INSERT INTO " . TOPICS_TABLE . " (topic_title, topic_poster, topic_time, forum_id, topic_status, topic_type, topic_vote) VALUES ('$post_subject', " . $userdata['customers_id'] . ", $current_time, $forum_id, " . TOPIC_UNLOCKED . ", $topic_type, $topic_vote)" : "UPDATE " . TOPICS_TABLE . " SET topic_title = '$post_subject', topic_type = $topic_type " . (($post_data['edit_vote'] || !empty($poll_title)) ? ", topic_vote = " . $topic_vote : "") . " WHERE topic_id = $topic_id";
		if (!$db->sql_query($sql))
		{
	...
	$sql = ($mode != "editpost") ? "INSERT INTO " . POSTS_TABLE . " (topic_id, forum_id, poster_id, post_username, post_time, poster_ip, enable_bbcode, enable_html, enable_smilies, enable_sig) VALUES ($topic_id, $forum_id, " . $userdata['customers_id'] . ", '$post_username', $current_time, '$user_ip', $bbcode_on, $html_on, $smilies_on, $attach_sig)" : "UPDATE " . POSTS_TABLE . " SET post_username = '$post_username', enable_bbcode = $bbcode_on, enable_html = $html_on, enable_smilies = $smilies_on, enable_sig = $attach_sig" . $edited_sql . " WHERE post_id = $post_id";
...
//
function update_post_stats(&$mode, &$post_data, &$forum_id, &$topic_id, &$post_id, &$customers_id)
{
	global $db;
...
function user_notification($mode, &$post_data, &$topic_title, &$forum_id, &$topic_id, &$post_id, &$notify_user)
{
	...
	if ($mode == 'delete')
	{
		$delete_sql = (!$post_data['first_post'] && !$post_data['last_post']) ? " AND customers_id = " . $userdata['customers_id'] : '';
	...
	else 
	{
		if ($mode == 'reply')
		{
			...
			while ($row = $db->sql_fetchrow($result))
			{
				...
				{
					$customers_id_sql .= ', ' . $row['ban_userid'];
				}
			}

			$sql = "SELECT u.customers_id, u.customers_email_address, u.user_lang 
				FROM " . TOPICS_WATCH_TABLE . " tw, " . USERS_TABLE . " u 
				WHERE tw.topic_id = $topic_id 
					AND tw.customers_id NOT IN (" . $userdata['customers_id'] . ", " . ANONYMOUS . $customers_id_sql . ") 
					AND tw.notify_status = " . TOPIC_WATCH_UN_NOTIFIED . " 
					AND u.customers_id = tw.customers_id";
			if (!($result = $db->sql_query($sql)))
			{
			...
			if ($row = $db->sql_fetchrow($result))
			{
				...
				do
				{
					...
					{
						$bcc_list_ary[$row['user_lang']][] = $row['customers_email_address'];
					}
					$update_watched_sql .= ($update_watched_sql != '') ? ', ' . $row['customers_id'] : $row['customers_id'];
				}
				while ($row = $db->sql_fetchrow($result));
			...
			if ($update_watched_sql != '')
			{
				$sql = "UPDATE " . TOPICS_WATCH_TABLE . "
					...
					WHERE topic_id = $topic_id
						AND customers_id IN ($update_watched_sql)";
				$db->sql_query($sql);
			}
		}
		...
		{
			$sql = "INSERT INTO " . TOPICS_WATCH_TABLE . " (customers_id, topic_id, notify_status)
