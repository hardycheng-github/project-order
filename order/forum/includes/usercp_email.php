...
{
	$customers_id = ( !empty($HTTP_GET_VARS[POST_USERS_URL]) ) ? intval($HTTP_GET_VARS[POST_USERS_URL]) : intval($HTTP_POST_VARS[POST_USERS_URL]);
}
else
{
...
{
	redirect(append_sid("login.$phpEx?redirect=profile.$phpEx&mode=email&" . POST_USERS_URL . "=$customers_id", true));
}

$sql = "SELECT username, customers_email_address, user_viewemail, user_lang  
	FROM " . USERS_TABLE . " 
...
if ( $result = $db->sql_query($sql) )
{
	...
	if ( $row['user_viewemail'] || $userdata['user_level'] == ADMIN )
	{
		...
		if ( isset($HTTP_POST_VARS['submit']) )
		{
			...
			if ( !$error )
			{
				...
				if ( $result = $db->sql_query($sql) )
				{
					...
					$emailer->from($userdata['customers_email_address']);
					$emailer->replyto($userdata['customers_email_address']);

					$email_headers = 'X-AntiAbuse: Board servername - ' . $server_name . "\n";
					$email_headers .= 'X-AntiAbuse: User_id - ' . $userdata['customers_id'] . "\n";
					$email_headers .= 'X-AntiAbuse: Username - ' . $userdata['username'] . "\n";
					$email_headers .= 'X-AntiAbuse: User IP - ' . decode_ip($user_ip) . "\n";

					$emailer->use_template('profile_send_email', $user_lang);
					$emailer->email_address($customers_email_address);
					...
					if ( !empty($HTTP_POST_VARS['cc_email']) )
					{
						...
						$emailer->email_address($userdata['customers_email_address']);
						$emailer->set_subject($subject);
		...
		$template->assign_vars(array(
			...
			'S_HIDDEN_FIELDS' => '', 
			'S_POST_ACTION' => append_sid("profile.$phpEx?mode=email&amp;" . POST_USERS_URL . "=$customers_id"), 
