...
while( list($var, $param) = @each($params) )
{
	...
	{
		$$var = ( !empty($HTTP_POST_VARS[$param]) ) ? dohtmlspecialchars($HTTP_POST_VARS[$param]) : dohtmlspecialchars($HTTP_GET_VARS[$param]);
	}
	else
	{
		$$var = '';
...
switch ( $mode )
{
	...
	case 'poll_delete':
		...
		$select_sql = ( !$submit ) ? ", t.topic_title, p.enable_bbcode, p.enable_html, p.enable_smilies, p.enable_sig, p.post_username, pt.post_subject, pt.post_text, pt.bbcode_uid, u.username, u.customers_id, u.user_sig" : '';
		$from_sql = ( !$submit ) ? ", " . POSTS_TEXT_TABLE . " pt, " . USERS_TABLE . " u" : '';
		$where_sql = ( !$submit ) ? "AND pt.post_id = p.post_id AND u.customers_id = p.poster_id" : '';
...
if ( $result = $db->sql_query($sql) )
{
	...
	if ( $mode == 'editpost' || $mode == 'delete' || $mode == 'poll_delete' )
	{
		...
		$post_data['poster_post'] = ( $post_info['poster_id'] == $userdata['customers_id'] ) ? true : false;
		...
		//
		if ( $post_info['poster_id'] != $userdata['customers_id'] && !$is_auth['auth_mod'] )
		{
...
{
	$html_on = ( $submit || $refresh ) ? ( ( !empty($HTTP_POST_VARS['disable_html']) ) ? 0 : TRUE ) : ( ( $userdata['customers_id'] == ANONYMOUS ) ? $board_config['allow_html'] : $userdata['user_allowhtml'] );
}
...
{
	$bbcode_on = ( $submit || $refresh ) ? ( ( !empty($HTTP_POST_VARS['disable_bbcode']) ) ? 0 : TRUE ) : ( ( $userdata['customers_id'] == ANONYMOUS ) ? $board_config['allow_bbcode'] : $userdata['user_allowbbcode'] );
}
...
{
	$smilies_on = ( $submit || $refresh ) ? ( ( !empty($HTTP_POST_VARS['disable_smilies']) ) ? 0 : TRUE ) : ( ( $userdata['customers_id'] == ANONYMOUS ) ? $board_config['allow_smilies'] : $userdata['user_allowsmile'] );
}
...
$attach_sig = ( $submit || $refresh ) ? ( ( !empty($HTTP_POST_VARS['attach_sig']) ) ? TRUE : 0 ) : ( ( $userdata['customers_id'] == ANONYMOUS ) ? 0 : $userdata['user_attachsig'] );
		...
		if ( $vote_info = $db->sql_fetchrow($result) )
		{
			...
			$sql = "SELECT * 
				...
				WHERE vote_id = $vote_id 
					AND vote_user_id = " . $userdata['customers_id'];
			...
			if ( !($row = $db->sql_fetchrow($result2)) )
			{
				...
				$sql = "INSERT INTO " . VOTE_USERS_TABLE . " (vote_id, vote_user_id, vote_user_ip) 
					VALUES ($vote_id, " . $userdata['customers_id'] . ", '$user_ip')";
				if ( !$db->sql_query($sql, END_TRANSACTION) )
				{
					message_die(GENERAL_ERROR, "Could not insert customers_id for poll", "", __LINE__, __FILE__, $sql);
				}
...
else if ( $submit || $confirm )
{
	...
	if ( $error_msg == '' )
	{
		if ( $mode != 'editpost' )
		{
			$customers_id = ( $mode == 'reply' || $mode == 'newtopic' ) ? $userdata['customers_id'] : $post_data['poster_id'];
			update_post_stats($mode, $post_data, $forum_id, $topic_id, $post_id, $customers_id);
		}
...
{
	$username = ( !empty($HTTP_POST_VARS['username']) ) ? dohtmlspecialchars(trim(stripslashes($HTTP_POST_VARS['username']))) : '';
	$subject = ( !empty($HTTP_POST_VARS['subject']) ) ? dohtmlspecialchars(trim(stripslashes($HTTP_POST_VARS['subject']))) : '';
	$message = ( !empty($HTTP_POST_VARS['message']) ) ? dohtmlspecialchars(trim(stripslashes($HTTP_POST_VARS['message']))) : '';

	$poll_title = ( !empty($HTTP_POST_VARS['poll_title']) ) ? dohtmlspecialchars(trim(stripslashes($HTTP_POST_VARS['poll_title']))) : '';
	...
	if ( !empty($HTTP_POST_VARS['poll_option_text']) )
	{
		while( list($option_id, $option_text) = @each($HTTP_POST_VARS['poll_option_text']) )
		{
			...
			{
				$poll_options[$option_id] = dohtmlspecialchars(trim(stripslashes($option_text)));
			}
		}
	}
	...
	{
		$poll_options[] = dohtmlspecialchars(trim(stripslashes($HTTP_POST_VARS['add_poll_option_text'])));
	}
...
else
{
	...
	else if ( $mode == 'quote' || $mode == 'editpost' )
	{
		...
		{
			$username = ( $post_info['customers_id'] == ANONYMOUS && !empty($post_info['post_username']) ) ? $post_info['post_username'] : '';
		}
	}
}
