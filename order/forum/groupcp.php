...
function generate_user_info(&$row, $date_format, $group_mod, &$from, &$posts, &$joined, &$poster_avatar, &$profile_img, &$profile, &$search_img, &$search, &$pm_img, &$pm, &$email_img, &$email, &$www_img, &$www, &$icq_status_img, &$icq_img, &$icq, &$aim_img, &$aim, &$msn_img, &$msn, &$yim_img, &$yim)
{
	...
	$poster_avatar = '';
	if ( $row['user_avatar_type'] && $row['customers_id'] != ANONYMOUS && $row['user_allowavatar'] )
	{
	...
	{
		$email_uri = ( $board_config['board_email_form'] ) ? append_sid("profile.$phpEx?mode=email&amp;" . POST_USERS_URL .'=' . $row['customers_id']) : 'mailto:' . $row['customers_email_address'];
	...
	$temp_url = append_sid("profile.$phpEx?mode=viewprofile&amp;" . POST_USERS_URL . "=" . $row['customers_id']);
	$profile_img = '<a href="' . $temp_url . '"><img src="' . $images['icon_profile'] . '" alt="' . $lang['Read_profile'] . '" title="' . $lang['Read_profile'] . '" border="0" /></a>';
	$profile = '<a href="' . $temp_url . '">' . $lang['Read_profile'] . '</a>';

	$temp_url = append_sid("privmsg.$phpEx?mode=post&amp;" . POST_USERS_URL . "=" . $row['customers_id']);
...
if ( isset($HTTP_POST_VARS['groupstatus']) && $group_id )
{
	...
	if ( $row['group_moderator'] != $userdata['customers_id'] && $userdata['user_level'] != ADMIN )
	{
...
else if ( isset($HTTP_POST_VARS['joingroup']) && $group_id )
{
	...
	$sql = "SELECT ug.customers_id, g.group_type
	...
	if (	$row = $db->sql_fetchrow($result) )
	{
		if ( $row['group_type'] == GROUP_OPEN )
		{
			do
			{
				if ( $userdata['customers_id'] == $row['customers_id'] )
				{
					$template->assign_vars(array(
						'META' => '<meta http-equiv="refresh" content="3;url=' . append_sid("index.$phpEx") . '">')
					);
	...
	$sql = "INSERT INTO " . USER_GROUP_TABLE . " (group_id, customers_id, user_pending) 
		VALUES ($group_id, " . $userdata['customers_id'] . ", 1)";
	if ( !($result = $db->sql_query($sql)) )
	{
		message_die(GENERAL_ERROR, "Error inserting user group subscription", "", __LINE__, __FILE__, $sql);
	}

	$sql = "SELECT u.customers_email_address, u.username, u.user_lang, g.group_name 
		FROM ".USERS_TABLE . " u, " . GROUPS_TABLE . " g 
		WHERE u.customers_id = g.group_moderator 
	...
	$emailer->email_address($moderator['customers_email_address']);
	...
	if ( $confirm )
	{
		...
		if ( $userdata['user_level'] != ADMIN && $userdata['user_level'] == MOD )
		{
			$sql = "SELECT COUNT(auth_mod) AS is_auth_mod 
				...
				WHERE ug.customers_id = " . $userdata['customers_id'] . " 
...
else if ( $group_id )
{
	...
	if ( $group_info = $db->sql_fetchrow($result) )
	{
		...
		if ( $group_moderator == $userdata['customers_id'] || $userdata['user_level'] == ADMIN )
		{
			$is_moderator = TRUE;
		}
		...
		if ( !empty($HTTP_POST_VARS['add']) || !empty($HTTP_POST_VARS['remove']) || isset($HTTP_POST_VARS['approve']) || isset($HTTP_POST_VARS['deny']) )
		{
			...
			if ( isset($HTTP_POST_VARS['add']) )
			{
				...
				$sql = "SELECT customers_id, customers_email_address, user_lang, user_level  
					FROM " . USERS_TABLE . " 
				...
				$sql = "SELECT ug.customers_id, u.user_level 
					FROM " . USER_GROUP_TABLE . " ug, " . USERS_TABLE . " u 
					WHERE u.customers_id = " . $row['customers_id'] . " 
						AND ug.customers_id = u.customers_id 
				...
				{
					$sql = "INSERT INTO " . USER_GROUP_TABLE . " (customers_id, group_id, user_pending) 
						VALUES (" . $row['customers_id'] . ", $group_id, 0)";
					if ( !$db->sql_query($sql) )
					{
				...
				if ( ( ( isset($HTTP_POST_VARS['approve']) || isset($HTTP_POST_VARS['deny']) ) && isset($HTTP_POST_VARS['pending_members']) ) || ( isset($HTTP_POST_VARS['remove']) && isset($HTTP_POST_VARS['members']) ) )
				{
					...
					if ( isset($HTTP_POST_VARS['approve']) )
					{
						...
						$sql = "UPDATE " . USER_GROUP_TABLE . " 
							...
							WHERE customers_id IN ($sql_in) 
								AND group_id = $group_id";
						$sql_select = "SELECT customers_email_address 
							FROM ". USERS_TABLE . " 
							WHERE customers_id IN ($sql_in)"; 
					}
					else if ( isset($HTTP_POST_VARS['deny']) || isset($HTTP_POST_VARS['remove']) )
					{
						if ( $group_info['auth_mod'] )
						{
							$sql = "SELECT ug.customers_id, ug.group_id 
								FROM " . AUTH_ACCESS_TABLE . " aa, " . USER_GROUP_TABLE . " ug 
								WHERE ug.customers_id IN  ($sql_in) 
									AND aa.group_id = ug.group_id 
									AND aa.auth_mod = 1 
								GROUP BY ug.customers_id, ug.group_id 
								ORDER BY ug.customers_id, ug.group_id";
							if ( !($result = $db->sql_query($sql)) )
							{
								message_die(GENERAL_ERROR, 'Could not obtain moderator status', '', __LINE__, __FILE__, $sql);
							}
							...
							if ( $row = $db->sql_fetchrow($result) )
							{
								...
								do
								{
									$group_check[$row['customers_id']][] = $row['group_id'];
								}
								while ( $row = $db->sql_fetchrow($result) );

								while( list($customers_id, $group_list) = @each($group_check) )
								{
									if ( count($group_list) == 1 )
									{
										$remove_mod_sql .= ( ( $remove_mod_sql != '' ) ? ', ' : '' ) . $customers_id;
									}
								}
								...
								if ( $remove_mod_sql != '' )
								{
									$sql = "UPDATE " . USERS_TABLE . " 
										SET user_level = " . USER . " 
										WHERE customers_id IN ($remove_mod_sql) 
					...
					if ( isset($HTTP_POST_VARS['approve']) )
					{
						...
						while ($row = $db->sql_fetchrow($result))
						{
							$bcc_list[] = $row['customers_email_address'];
						}
	...
	//
	$sql = "SELECT username, customers_id, user_viewemail, user_posts, user_regdate, user_from, user_website, customers_email_address, user_icq, user_aim, user_yim, user_msnm  
		FROM " . USERS_TABLE . " 
		WHERE customers_id = " . $group_info['group_moderator'];
	if ( !($result = $db->sql_query($sql)) )
	{
	...
	//
	$sql = "SELECT u.username, u.customers_id, u.user_viewemail, u.user_posts, u.user_regdate, u.user_from, u.user_website, u.customers_email_address, u.user_icq, u.user_aim, u.user_yim, u.user_msnm, ug.user_pending 
		FROM " . USERS_TABLE . " u, " . USER_GROUP_TABLE . " ug
		WHERE ug.group_id = $group_id
			AND u.customers_id = ug.customers_id
			AND ug.user_pending = 0 
			AND ug.customers_id <> " . $group_moderator['customers_id'] . " 
		ORDER BY u.username"; 
	if ( !($result = $db->sql_query($sql)) )
	{
		message_die(GENERAL_ERROR, 'Error getting user list for group', '', __LINE__, __FILE__, $sql);
	...
	$sql = "SELECT u.username, u.customers_id, u.user_viewemail, u.user_posts, u.user_regdate, u.user_from, u.user_website, u.customers_email_address, u.user_icq, u.user_aim, u.user_yim, u.user_msnm
	...
	if ( $members_count )
	{
		...
		{
			if ( $group_members[$i]['customers_id'] == $userdata['customers_id'] && $userdata['session_logged_in'] )
			{
	...
	if ( $modgroup_pending_count )
	{
		...
		{
			if ( $modgroup_pending_list[$i]['customers_id'] == $userdata['customers_id'] && $userdata['session_logged_in'] )
			{
	...
	if ( $userdata['customers_id'] == $group_info['group_moderator'] )
	{
		$is_moderator = TRUE;
	...
	else if ( $userdata['customers_id'] == ANONYMOUS )
	{
	...
	$customers_id = $group_moderator['customers_id'];
	...
	$template->assign_vars(array(
		...
		'U_MOD_VIEWPROFILE' => append_sid("profile.$phpEx?mode=viewprofile&amp;" . POST_USERS_URL . "=$customers_id"), 
	...
	for($i = $start; $i < min($board_config['topics_per_page'] + $start, $members_count); $i++)
	{
		...
		$customers_id = $group_members[$i]['customers_id'];
		...
		if ( $group_info['group_type'] != GROUP_HIDDEN || $is_group_member || $is_moderator )
		{
			...
			$template->assign_block_vars('member_row', array(
				...
				'U_VIEWPROFILE' => append_sid("profile.$phpEx?mode=viewprofile&amp;" . POST_USERS_URL . "=$customers_id"))
			);
	...
	if ( $is_moderator )
	{
		...
		if ( $modgroup_pending_count )
		{
			for($i = 0; $i < $modgroup_pending_count; $i++)
			{
				...
				$customers_id = $modgroup_pending_list[$i]['customers_id'];
				...
				$user_select = '<input type="checkbox" name="member[]" value="' . $customers_id . '">';
...
else
{
	...
	if ( $userdata['session_logged_in'] ) 
	{
		$sql = "SELECT g.group_id, g.group_name, g.group_type, ug.user_pending 
			...
			ORDER BY g.group_name, ug.customers_id";
		if ( !($result = $db->sql_query($sql)) )
		{
			message_die(GENERAL_ERROR, 'Error getting group information', '', __LINE__, __FILE__, $sql);
		}
