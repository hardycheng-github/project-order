...
/* -----------------------------------------------------------------------------------------
  ...
  if ($error == 1) {
    twe_redirect(twe_href_link(FILENAME_ADVANCED_SEARCH, 'errorno=' . $errorno . '&' . twe_get_all_get_params(array('x', 'y'))));
  } else {
    $breadcrumb->add(NAVBAR_TITLE1_ADVANCED_SEARCH, twe_href_link(FILENAME_ADVANCED_SEARCH));
