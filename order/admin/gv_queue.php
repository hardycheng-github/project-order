...
<?php
  ...
  switch ($_GET['action']) {
    case 'release':
      ...
      $contents[] = array('align' => 'center', 'text' => '<a href="' . twe_href_link('gv_queue.php','action=confirmrelease&gid='.$gInfo->unique_id,'NONSSL').'">'.twe_image_button('button_confirm_red.gif', IMAGE_CONFIRM) . '</a> <a href="' . twe_href_link('gv_queue.php','action=cancel&gid=' . $gInfo->unique_id,'NONSSL') . '">' . twe_image_button('button_cancel.gif', IMAGE_CANCEL) . '</a>');
      break;
    default:
      $heading[] = array('text' => '[' . $gInfo->unique_id . '] ' . twe_datetime_short($gInfo->date_created) . ' ' . $currencies->format($gInfo->amount));

      $contents[] = array('align' => 'center','text' => '<a href="' . twe_href_link('gv_queue.php','action=release&gid=' . $gInfo->unique_id,'NONSSL'). '">' . twe_image_button('button_release.gif', IMAGE_RELEASE) . '</a>');
      break;
   }
