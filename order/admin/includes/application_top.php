...
//START MASTER PRODUCTS
  ...
  require (DIR_FS_CATALOG . DIR_WS_CLASSES . 'adodb4990/adodb-errorhandler.inc.php');
  require (DIR_FS_CATALOG . DIR_WS_CLASSES . 'adodb4990/adodb.inc.php');
 ...
 if (STORE_SESSIONS != 'mysql')  session_save_path(SESSION_WRITE_DIRECTORY);
  ...
  if (SESSION_CHECK_IP_ADDRESS == 'True') {
    ...
    if (!twe_session_is_registered('SESSION_IP_ADDRESS')) {
...
// $current_page = preg_split('\?', basename($_SERVER['PHP_SELF'])); $current_page = $current_page[0]; // for BadBlue(Win32) webserver compatibility
  $current_page = preg_split('/\?/i', basename($_SERVER['PHP_SELF'])); $current_page = $current_page[0]; // for BadBlue(Win32) webserver compatibility
...
if (!strstr($PHP_SELF, 'Adminlogin.php')) {
  ...
  require(DIR_FS_CATALOG.DIR_WS_CLASSES . 'Smarty_2.6.19/Smarty.class.php');
  ?>
