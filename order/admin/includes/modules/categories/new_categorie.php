...
/* --------------------------------------------------------------
   ...
   --------------------------------------------------------------*/
    if ( ($_GET['cID']) && (!$_POST) ) {
	  ...
	  $categories_name_size = $_POST['categories_name_size'];
...
?>
    ...
    echo twe_draw_form('new_category', FILENAME_CATEGORIES, 'cPath=' . $cPath . '&cID=' . $_GET['cID'] . '&action='.$form_action, 'post', 'enctype="multipart/form-data"'); ?>
...
<tr>
            ...
            <td class="main"><?php echo twe_draw_file_field('categories_image') . '<br>' . twe_draw_separator('pixel_trans.gif', '24', '15') . '&nbsp;' .  twe_draw_hidden_field('categories_previous_image', $cInfo->categories_image); ?>
			 <?php
            ...
            echo twe_draw_selection_field('del_cat_pic', 'checkbox', 'yes').TEXT_DELETE;
...
echo '<td><span class="main">'.twe_draw_pull_down_menu('categories_template',$files,$default_value);
?>
        </span></td>
      </tr>
      <tr>
...
?>
          ...
          <tr>
            ...
            <td class="main"><?php echo twe_draw_input_field('sort_order', $cInfo->sort_order, 'size="2"').twe_draw_hidden_field('categories_status', $cInfo->categories_status); ?></td>
          </tr>
<?php
...
<?php    for ($i=0; $i<sizeof($languages); $i++) { ?>
          <tr>
            ...
            <td class="main">
			<?php echo twe_image(DIR_WS_LANGUAGES.$languages[$i]['directory'].'/admin/images/'.$languages[$i]['image']) . '&nbsp;' . twe_draw_input_field('categories_name[' . $languages[$i]['id'] . ']', (($categories_name[$languages[$i]['id']]) ? stripslashes($categories_name[$languages[$i]['id']]) : twe_get_categories_name($cInfo->categories_id, $languages[$i]['id']))); ?>
			</td>
          </tr>
<?php } ?>
...
<?php    for ($i=0; $i<sizeof($languages); $i++) { ?>
          <tr>
			<td class="main"><?php if ($i == 0) echo TEXT_EDIT_CATEGORIES_NAME_SIZE; ?></td>
			<td class="main">
			<?php echo twe_image(DIR_WS_LANGUAGES.$languages[$i]['directory'].'/admin/images/'.$languages[$i]['image']) . '&nbsp;' . twe_draw_input_field('categories_name_size[' . $languages[$i]['id'] . ']', (($categories_name_size[$languages[$i]['id']]) ? stripslashes($categories_name_size[$languages[$i]['id']]) : twe_get_categories_name_size($cInfo->categories_id, $languages[$i]['id'])), ' min="8" max="36"', false, 'number'). 'pt'; ?>
			</td>
          </tr>
<?php } ?>
...
<?php    for ($i=0; $i<sizeof($languages); $i++) { ?>
          ...
          <tr>
            ...
            <td><table border="0" cellspacing="0" cellpadding="0">
              <tr>
                <td class="main" valign="top"><?php echo twe_image(DIR_WS_LANGUAGES.$languages[$i]['directory'].'/admin/images/'.$languages[$i]['image']); ?>&nbsp;</td>
                <td class="main"><?php echo twe_draw_textarea_field('categories_meta_title[' . $languages[$i]['id'] . ']', 'soft', '70', '2', (($categories_meta_title[$languages[$i]['id']]) ? stripslashes($categories_meta_title[$languages[$i]['id']]) : twe_get_categories_meta_title($cInfo->categories_id, $languages[$i]['id']))); ?></td>
              </tr>
            </table></td>
          </tr>
          <tr>
           ...
           <tr>
            ...
            <td><table border="0" cellspacing="0" cellpadding="0">
              <tr>
                <td class="main" valign="top"><?php echo twe_image(DIR_WS_LANGUAGES.$languages[$i]['directory'].'/admin/images/'.$languages[$i]['image']); ?>&nbsp;</td>
                <td class="main"><?php echo twe_draw_textarea_field('categories_meta_description[' . $languages[$i]['id'] . ']', 'soft', '70', '3', (($categories_meta_description[$languages[$i]['id']]) ? stripslashes($categories_meta_description[$languages[$i]['id']]) : twe_get_categories_meta_description($cInfo->categories_id, $languages[$i]['id']))); ?></td>
              </tr>
            </table></td>
          </tr>
          <tr>
           ...
           <tr>
            ...
            <td><table border="0" cellspacing="0" cellpadding="0">
              <tr>
                <td class="main" valign="top"><?php echo twe_image(DIR_WS_LANGUAGES.$languages[$i]['directory'].'/admin/images/'.$languages[$i]['image']); ?>&nbsp;</td>
                <td class="main"><?php echo twe_draw_textarea_field('categories_meta_keywords[' . $languages[$i]['id'] . ']', 'soft', '70', '3', (($categories_meta_keywords[$languages[$i]['id']]) ? stripslashes($categories_meta_keywords[$languages[$i]['id']]) : twe_get_categories_meta_keywords($cInfo->categories_id, $languages[$i]['id']))); ?></td>
              </tr>
            </table></td>
          </tr>
<?php } ?>
      ...
      <tr>
        <td class="main" align="right"><?php echo twe_draw_hidden_field('categories_date_added', (($cInfo->date_added) ? $cInfo->date_added : date('Y-m-d'))) . twe_draw_hidden_field('parent_id', $cInfo->parent_id) . twe_image_submit('button_save.gif', IMAGE_SAVE,'style="cursor:hand" onClick="return confirm(\''.SAVE_ENTRY.'\')"') . '&nbsp;&nbsp;<a href="' . twe_href_link(FILENAME_CATEGORIES, 'cPath=' . $cPath . '&cID=' . $_GET['cID']) . '">' . twe_image_button('button_cancel.gif', IMAGE_CANCEL) . '</a>'; ?></td>
      </form></tr>